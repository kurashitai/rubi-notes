import { huntsDatabaseIntibia } from '../src/data/hunts-intibia';

const entries = Object.entries(huntsDatabaseIntibia);
...
const pct = (n: number) => `${n}/${totals.total} (${((n / totals.total) * 100).toFixed(1)}%)`;
...
const showSome = (label: string, arr: string[]) => {
